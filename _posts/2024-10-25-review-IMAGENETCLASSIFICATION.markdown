...
논문에서 사용한 두가지의 과적합 방지 방식을 알아보자

##### 4.1 Data Augmentation
이미지 데이터에 대해 과적합을 줄이기 위한 가장 쉬운 방법은 label-preserving transformations를 사용해 데이터셋을 인위적으로 늘리는 것이다. 
첫번째 데이터 증강 방식은 이미지 변환과 수평반사를 생성하는 것이다. 256 x 256 사이즈의 이미지로부터 무작위 224 x 224 patch (및 수평반사)를 추출하고 이 추출된 이미지로 네트워크를 학습시킨다. test 시에 네트워크는 네 개의 코너와 중간 패치 그리고 그것들을 전부 수평반사 시켜 총 10개의 패치로 예측을 한다 그 후 10개의 패치들에 대한 softmax layer에 의해 만들어진 예측들을 평균내린다.
두번째 데이터 증강 방식은 훈련 이미지에서 RGB 채널의 강도를 변경하는 것이다. ImageNet 훈련 세트 전체에 걸쳐 RGB 픽셀 값 세트에 대한 PCA를 수행한다. 각 훈련 이미지에 해당 고유값에 평균이 0이고 표준편차가 0.1인 가우시안 분포에서 추출한 무작위 변수에 해당 고유값에 비례하는 크기를 가진 주요 구성 요소의 배수를 더한다.

##### 4.2 Dropout
많은 다양한 모델로부터의 예측을 합치는 것은 test error를 줄이는 데 마우 성공적인 방식이다. 하지만 이미 며칠씩이나 걸리는 대규모 신경망에는 비용이 너무 많이 든다. 훈련 중에 약 2배의 비용만 드는 매우 효율적인 버전의 모델 조합이 있다. Dropout은 각각의 은닉 뉴런들의 출력을 0.5의 확률로 0으로 만들어버린다.
첫 두 fc layer에 dropout을 적용했다. 
